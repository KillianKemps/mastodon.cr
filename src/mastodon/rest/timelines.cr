...
module Mastodon
  module REST
    module Timelines
      ...
      def timeline_home(limit = Statuses::DEFAULT_STATUSES_LIMIT)
      ...
      def timeline_public(local = false, limit = Statuses::DEFAULT_STATUSES_LIMIT)
        params = HTTP::Params.build do |param|
      ...
      def timeline_tag(hashtag, local = false, limit = Statuses::DEFAULT_STATUSES_LIMIT)
        params = HTTP::Params.build do |param|
          param.add "local", "" if local
          param.add "limit", "#{limit}" if limit != Statuses::DEFAULT_STATUSES_LIMIT && limit <= 80
        end
