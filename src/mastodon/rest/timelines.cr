...
module Mastodon
  module REST
    module Timelines
      ...
      def timeline_home(limit = DEFAULT_STATUSES_LIMIT)
      ...
      def timeline_public(local = false, limit = DEFAULT_STATUSES_LIMIT)
        params = HTTP::Params.build do |param|
      ...
      def timeline_tag(hashtag, local = false, limit = DEFAULT_STATUSES_LIMIT)
        params = HTTP::Params.build do |param|
          param.add "local", "" if local
          param.add "limit", "#{limit}" if limit != DEFAULT_STATUSES_LIMIT && limit <= 80
        end
